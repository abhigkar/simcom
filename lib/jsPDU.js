/**
 * pduDecoder
 *  - Modified library for node js, removed HTML interactions only
 *  - This node module is based on https://github.com/tladesignz/jsPduDecoder
 ...
 */
jsPDU.decode = function ( pdu ) {
	var i,result = [], decodedMessage = {};
	...
	for (i = 0; i < tokens.length; ++i) {
		result.push( tokens[ i ](decodedMessage) );
	}

	return decodedMessage;
}

function fillBucket(bucket, name, value){
	bucket[name] = value;
	return bucket;
}
...
function tokenizer( octets ) {
	...
	if (smscLength) {
		...
		tokenList.push( function(message){ return fillBucket(message,'smsc',tokens.Number( sliceSmsc, undefined, tokens.ToA( sliceSmscToA ) ));} );
		tokenList.push( function(message){ return fillBucket(message,'smsc_info',tokens.ToA( sliceSmscToA ).info);} );
	}

	// Sender/Receiver part
	pos = smscLength + 1;
	var pduType = tokens.ToM( octets[ pos ] );
	tokenList.push( function(message){ return fillBucket(message,'pdu_type',pduType.info);} );
	...
	if (pduType.type === 'deliver') {
		...
		if (numberLength) {
			...
			tokenList.push( function(message){ return fillBucket(message,'sender',tokens.Number( sliceNumber, numberLength, tokens.ToA( sliceNumberToA ) ));} );
			tokenList.push( function(message){ return fillBucket(message,'sender_info',tokens.ToA( sliceNumberToA ).info); } );
		...
		tokenList.push( function(message){ return fillBucket(message,'time',tokens.SCTS( sliceTimeStamp )); } );
	...
	if (pduType.TP_UDHI) {
		...
		tokenList.push( function(message) { 
			message['udh'] = message['udh'] || {};
			return fillBucket(message.udh,'length',TP_UDHL.length); 
		} );
		...
		tokenList.push( function(message) { 
			message['udh'] = message['udh'] || {};
			let tMessahe = fillBucket(message.udh,'reference_number',TP_UDH.ieiInfo.referenceNumber);
			fillBucket(tMessahe,'currrent_part',TP_UDH.ieiInfo.currrentPart);
			return fillBucket(tMessahe,'parts',TP_UDH.ieiInfo.parts); 
		} );
	...
	if (TP_UDH.wap) {
		...
		tokenList.push( function(message){ return fillBucket(message,'text',tokens.UD( sliceMessage, TP_DCS.alphabet, TP_UDHL.padding, TP_UDH.formatting )); } );
...
var tokens = {
	...
	UDH: function( octets ) {
		...
		var firstIEI = {};
		if(IEs.length > 0 && IEs[0].IEI === 0){
			firstIEI.referenceNumber = IEs[0].IED[0];
			firstIEI.parts = IEs[0].IED[1];
			firstIEI.currrentPart = IEs[0].IED[2];
			firstIEI.iei = IEs[0].IEI;
		}
		 
		return {wap: isWap, formatting: formatting, info: info.join( '; ' ),ieiInfo: firstIEI};
	},
