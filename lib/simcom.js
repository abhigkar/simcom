...
*/
SimCom.prototype.readSMS = function(index, mode) {
  return this.invoke('AT+CMGR=' + index + ',' +  (mode ? 0 : 1), function(res) {
		let raw = res.shift().pdu;
		return {
			parsedMessage: jspdu.decode(raw),
			pdu: raw
		};
  }, true);
}
...
/*peek
0 = Default specified the index
1 = Delete all read from  preferred storage and leave unread and draft/unsent 
2 = Delete all read from  preferred storage, sent and leave unread and unsent 
3 = Delete all read from  preferred storage, sent, unsent and leave unread
4 = Delete all  from  preferred storage
*/
SimCom.prototype.deleteSMS = function(index, peek){
  peek = peek || 0;
  return this.invoke('AT+CMGD=' + index + ',' +  peek);
}
...
/*
 ...
 6 Delete all SMS 
*/
SimCom.prototype.deleteAllSMS = function(stat){
  stat = stat || 0;
  return this.invoke('AT+CMGDA=' + stat);
}
...
SimCom.prototype.preferredStorage = function() {
  return this.invoke('AT+CPMS?', function(res) {
    console.log(res);
  });
}
